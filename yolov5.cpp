...
//            cv::Mat img = cv::imread(img_dir + "/" + file_names[f - fcount + 1 + b]); //读images
//            if (img.empty()) continue;
//
//            imgs_buffer[b] = img;//存images
//
//            size_t  size_image = img.cols * img.rows * 3;
//            size_t  size_image_dst = INPUT_H * INPUT_W * 3;
//            //copy data to pinned memory  将数据复制到固定内存
//            memcpy(img_host,img.data,size_image);
//            //copy data to device memory  将数据复制到设备内存
